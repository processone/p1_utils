    ...
    R = #opts{root = get_attrval(root, Element),
	      ...
	      sort_functions = proplists:get_value(sort_functions,
              ...
              pretty_printer = proplists:get_value(pretty_printer,
                                                   Options, ''),
              image = proplists:get_value(image,
                                          Options, "erlang.png"),
              link = proplists:get_value(link,
                                          Options, "http://www.erlang.org/")
	     },
...
layout_module(#xmlElement{name = module, content = Es}=E, Opts) ->
    ...
    Functions = [{function_name(E1), E1} || E1 <- get_content(functions, Es)],
    Types = [{type_name(E1), E1} || E1 <- get_content(typedecls, Es)],
    SortedFs = lists:sort(Functions),
    Body = (navigation("top", Opts)
...
image(Opts) ->
    {td, [{a, [{href, Opts#opts.link}, {target, "_top"}],
           [{img, [{src, Opts#opts.image}, {align, "right"},
                   {border, 0}, {alt, "Logo"}],
             []}]}
         ]}.

navigation(Where, Opts) ->
    [?NL,
     {'div', [{class, "navbar"}],
